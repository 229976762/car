	...
	<view class="components-theme">
			...
			<block>
				<view class='text-center'>
					...
					<view class="cu-avatar2 round xl margin-right-sm shadow-blur-lg bg-img open-data"
						...
						<button class="cu-avatar xl round" open-type="chooseAvatar" @chooseavatar="onChooseAvatar">
							<image :src="avatar" style="width: 100%; height: 100%;"></image>
						</button>
					</view>
					<view class="padding text-blue text-xl text-bold" @click="handleToLogin">
						<input v-model="nickName"  placeholder="点击登录"/>
					</view>
		...
		<block>
			...
			<view class="cu-list menu card-menu margin-top-lg shadow-shop bg-white text-black my-radius sm-border">
				...
				<view class="cu-item ">
					<button class='content cu-btn'>
						...
						<text class='text-lg margin-sm'>账户余额</text>
					</button>
					<view class="action">
						 <view class="cu-tag round bg-orange light">{{balance}} 元</view>
					</view>
				</view>
			...
			<view class="cu-list menu card-menu margin-bottom-lg shadow-shop bg-white text-black my-radius sm-border">
				<view class="cu-item">
					<button class='content cu-btn' @click="mentalTest">
						<image src='../../static/me/icon/dengta.png' class='png' mode='aspectFit'></image>
						<text class='text-lg margin-sm'>充值</text>
					</button>
				</view>
				<view class="cu-item" @click="callPhoneNumber" data-number="925-5487-521">
					<view class='content'>
						<image src='../../static/me/icon/chucuo.png' class='png' mode='aspectFit'></image>
						<text class='text-lg margin-sm'>客服呼叫</text>
					</view>
					<view class="action">
						<view class="cu-tag round bg-blue light">925-5487-521</view>
					</view>
				</view>
				<view class="cu-item">
					<button class='content cu-btn' @click="goAboutMe">
						<image src='../../static/me/icon/xiaoxi.png' class='png' mode='aspectFit'></image>
						<text class='text-lg margin-sm'>关于小程序</text>
					</button>
				</view>
			</view>
		...
		<view v-if="isLogin" class="menu ">
		  <view class="cu-item item-box">
			<view   class="content text-center" @click="handleLogout">
			  <text class="padding text-red text-xl text-bold">退出登录</text>
			</view>
		  </view>
		</view>


		<view class="cu-modal" :class="showModal1?'show':''">
			<view v-show="showModal1" class="action modal-overlay" @tap="cancelInput"></view>
		    <view class="cu-dialog" style="z-index: 10000;">
		        <view class="cu-bar bg-white justify-end">
		            <view class="content text-black text-xl text-bold">充值信息</view>
		        ...
		        <view style="margin-top: 10rpx;" class="cu-list menu">
					<view class="cu-item">
						...
						<view class="action">
							<input v-model="payment" class="input" placeholder="请输入充值金额（人民币）" style="text-align: left;"></input>
						</view>
					</view>
		            
		            <p @click="confirmInput" class="btn-confirm">
		                <text class="text-blue text-xl text-bold">[确认充值]</text>
		            </p>
...
<script>
	...
	import { handleToLogin } from '@/common/handeLogin'
	...
	export default {
		data() {
			return {
				...
				nickName: uni.getStorageSync('user_info').name,
				isLogin: uni.getStorageSync("token") !== null,
				balance: 0,
				inter: [{
					title: 'mimicry',
					name: '活力春天',
		...
		methods: {
			...
			onChooseAvatar(e) {
					...
					wx.uploadFile({
							...
							url: 'http://localhost:9999/admin/auth/oauth2/token',//服务器端接收图片的路径
							success:function(res){
							console.log(res);//发送成功回调
								// that.$store.commit('SET_AVATAR', res.data.newAvatar)
								resolve(res);
							},
							fail:function(res){
			...
			confirmInput() {
			    this.balance = this.balance + Number.parseInt(this.payment)
				let opts = {
					url: `/sysUserBalance`,
					method: 'put',
					data: this.balance
